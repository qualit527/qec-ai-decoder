...
这部分很重要。你的工作不是把需求一次性说给 Agent，然后等它“自动完成项目”。

更现实的做法是：
- 你先给 Agent 一个边界清楚的小任务。
- Agent 完成后，你人工检查结果和接口。
- 你再根据检查结果发下一轮 prompt。

你要把自己理解成“技术负责人 + 审核人”，不是“纯转述需求的人”。

### 阶段 A: Day 1 先打通 `surface_d5` 环境
...
2. 补齐 surface_d5_depol 所需的最小实现
3. 不要处理 bb72、verify、review-log 相关内容
4. 修改后告诉我：
   - 你改了哪些文件
   - 还缺什么
   - 哪些点需要我人工拍板

优先级：先保证 baseline 跑通，再考虑后续封装。
```

Agent 做完后，你人工要做的事：
- 看它改了哪些文件，确认没有顺手改到 `bb72`、verify 或别人负责的区域。
- 检查 `surface_d5_depol` 的命名、字段和设计稿是否一致。
- 确认 baseline 路径是不是清楚，比如输入是什么、输出是什么、结果落在哪里。
- 把需要冻结的字段记下来，准备同步给Lin Tengxiang和Xie Jingu。

第二轮再发给 Agent 的 prompt：

```text
继续在刚才的实现上，把 surface_d5 的 syndrome 数据流和 PyMatching baseline 真正跑通。

要求：
1. 说明从 circuit -> syndrome / detection events -> decoder input 的路径
2. 给出最小可复现运行命令
3. 输出一份接口字段清单，供我同步给 Runner 和 verify 负责人
4. 如果有不确定的字段，明确标成“需要人工决定”
```

这轮做完后，你人工要做的事：
- 亲自看一遍路径是不是闭合的，不要只相信“它说跑通了”。
- 把字段清单发给Lin Tengxiang和Xie Jingu，要求两边确认。
- 如果有命名冲突，这一步就拍板，不要拖到 Day 2。
...
```text
现在开始做 orchestration 最小闭环。

请基于当前代码实现一轮最小流程：
env -> ideator/coder/analyst stub -> runner -> metrics/log 落盘

要求：
1. 即使先用 stub / mock，也要把目录结构和接口固定下来
2. 输出 round_<N>/ 下应该有哪些文件
3. 告诉我哪些接口已经冻结，哪些还需要人工协调
4. 不要顺手重构 Runner 内部逻辑
```

Agent 做完后，你人工要做的事：
- 打开 `round_<N>/` 看目录和文件是不是和设计稿一致。
- 检查 `metrics.json`、`log.md`、`history.jsonl` 是否足够给 verify 使用。
- 拉上Lin Tengxiang、Xie Jingu做一次接口确认，尤其是字段名和落盘位置。

如果这一步发现接口不一致，再发下一轮 prompt：

```text
根据下面这些人工确认后的接口约束修正 orchestration：

[把你们三个人刚确认的字段和目录约束贴进来]
...
### 阶段 C: Day 3 交付 Demo 1 和入口

发给 Agent 的 prompt：

```text
请把当前可运行流程整理成 Demo 1 和 /autoqec-run 的最小可演示版本。
...
3. walkthrough 草稿
4. 我演示时必须人工讲解的三句话
5. 如果运行失败，最保守的 fallback 是什么
```

Agent 做完后，你人工要做的事：
- 真正按它给的命令走一遍演示流程。
- 把 walkthrough 改成你自己能讲出口的版本，不要原样念模型生成的文案。
- 记录 1 到 2 个最可能现场出问题的点，并准备 fallback 讲法。

如果演示前你发现命令太复杂或输出太散，再发最后一轮 prompt：

```text
请把 Demo 1 的演示路径再压缩一次。

要求：
1. 命令尽量少
2. 输出尽量集中到一个 run 目录
3. 给我一份 1 分钟版本的口头讲解提纲
4. 不改核心逻辑，只做演示友好化整理
```

## 3. 你和另外两个人怎么配合

- 你要和Lin Tengxiang对齐 Runner 接口，因为你这边的 orchestrator 最终要调用他负责的训练与评估主流程。
- 你要和Xie Jingu对齐 `metrics.json` 和 verify 所需字段，因为后续验证模块会直接吃你这里落盘的数据。
