...
env PYTHON_BIN="$PYTHON_BIN" bash demos/demo-5-failure-recovery/run.sh > "$LOG_DIR/demo5.stdout" 2> "$LOG_DIR/demo5.stderr"
exit_code=$?
end=$("$PYTHON_BIN" - <<'PY'
import time
print(f"{time.time():.6f}")
...
print(json.dumps([{
    "name": "demo5",
    "cmd": "bash demos/demo-5-failure-recovery/run.sh",
    "exit_code": int(os.environ["EXIT_CODE"]),
    "elapsed_s": float(os.environ["END"]) - float(os.environ["START"]),
    "stdout": "runs/demo-5-showcase/logs/demo5.stdout",
...
print("  python -m http.server 8767 --bind 127.0.0.1")
print("Then open:")
print("  http://127.0.0.1:8767/runs/demo-5-showcase/report.html")
PY
exit "$report_exit"
